    ...
    while (1){  // 主循环
        ...
        if (ADS805_Data_Completed) { // ADC数据采集完成
            ...
            for(int i = 0; i < FFT_ADC_N/2; i++){
                if(fft_out_adc_data[i] > 2500){//找大于2500的值
            ...
            // 判断信号类型
            if(comper >= 3){//值大于等于三个时判断为三角波与正弦波的复合信号
			...
			if(signal_type) {
				printf("信号类型: 复合信号 (");
				printf(signal_mode == 0 ? "三角波与正弦波组合，以三角波为主)\n" : "三角波与正弦波组合，以正弦波为主)\n");
			} 
			else {
				printf("信号类型: 简单信号 (两个");
				if(comper == 2) {
					printf(fft_out_adc_data[comper_array[0]] > FFT_TRI_BASE ? "三角波" : "正弦波");
					printf("组合，频率不同)\n");
				} 
				else if(comper == 1) {
					printf(fft_out_adc_data[comper_array[0]] > FFT_TRI_BASE ? "三角波" : "正弦波");
			...
			// 输出相位差(转换为角度)
			float phase_deg = phase * 180.0 / PI;
			printf("相位差: %.2f 度\n", phase_deg);
			 // 输出频率成分
			printf("检测到的频率成分[索引, 频率(Hz), 幅值]:\n");
			for(int i = 0; i < comper; i++) {
				int freq_index = comper_array[i];
				float frequency = (float)freq_index * (f_s / FFT_ADC_N); // 需要替换为您的实际采样率
				printf("[%d] %.2f Hz, %.2f", freq_index, frequency, fft_out_adc_data[freq_index]);
				
				// 对复合信号标注波形类型
				if(signal_type) {
					printf(fft_out_adc_data[freq_index] > FFT_TRI3_BASE ? " (三角波)\n" : " (正弦波)\n");
				} else {
					printf("\n");
				}
			}
			...
			// 输出主频率信息
			float main_freq = (float)fft_max_value_index_1 * (f_s / FFT_ADC_N);
			printf("主频率: %.2f Hz (索引:%d, 幅值:%.2f)\n", main_freq, fft_max_value_index_1, fft_max_value_1);
